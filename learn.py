...
class TD_search(object):
    ...
    def __init__(self, env, agent, gamma=0.9, search_time=0.01, memsize=2000, batch_size=256, temperature=1):
        """
    ...
    def play_game(self, k, maxiter=80):
        ...
        while not episode_end:
            ...
            if game_node:
                game_node = game_node.add_variation(max_move)
            else:
                game_node = game.add_variation(max_move)
        ...
        game_storage = open("game_storage.txt", "a")
        game_storage.write(game.__str__() + "\n\n")
        game_storage.close()
        return self.env.board
    ...
    def mcts(self, node):
        ...
        while starttime + self.search_time > time.time() and sim_count < self.min_sim_count:
            depth = 0
            color = 1
            node_rewards = []
